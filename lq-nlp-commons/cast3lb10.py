...
class Cast3LBn(cast3lb.Cast3LB):
    ...
    def simplify_tags(self):
        map(lambda t: t.map_leaves(self.tag_filter), self.trees)
    ...
    def simplify_tags_more(self):
        map(lambda t: t.map_leaves(self.tag_filter_more), self.trees)
...
class Cast3LBPn(Cast3LBn):
    ...
    def _generate_trees(self):
        print "Parsing Cast3LB treebank..."
        f = lambda t: len(filter(lambda x: not cast3lb.is_punctuation(x), t.leaves())) <= self.n
        m = lambda t: self._prepare(t)
        trees = [t for t in itertools.ifilter(f, itertools.imap(m, self.parsed()))]
        return trees
